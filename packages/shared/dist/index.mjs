...
var ProjectSchema = z2.object({
  ...
  visibility: z2.enum([VISIBILITY.private, VISIBILITY.public]).default(VISIBILITY.private),
...
var SegmentSchema = z3.object({
  id: z3.string().uuid(),
  project_id: z3.string().uuid(),
  name: z3.string().min(1).max(100),
  position: z3.number().int().min(0).default(0),
  created_at: z3.coerce.date(),
  updated_at: z3.coerce.date()
}).strict();
...
var PromptSchema = z4.object({
  id: z4.string().uuid(),
  segment_id: z4.string().uuid(),
  title: z4.string().min(1).max(200),
  body: z4.string().min(1),
  language: z4.enum([LANGUAGE.en, LANGUAGE["pt-BR"]]).default(DEFAULT_LANGUAGE),
  kind: z4.enum([KIND.prompt, KIND.system, KIND.tool]).default(KIND.prompt),
  position: z4.number().int().min(0).default(0),
...
var ValidatorSchema = z5.object({
  id: z5.string().uuid(),
  prompt_id: z5.string().uuid(),
  title: z5.string().min(1).max(200),
  body: z5.string().min(1),
  created_at: z5.coerce.date(),
  updated_at: z5.coerce.date()
}).strict();
...
var VersionSchema = z6.object({
  id: z6.string().uuid(),
  entity_type: z6.enum([ENTITY_TYPE.project, ENTITY_TYPE.segment, ENTITY_TYPE.prompt, ENTITY_TYPE.validator]),
  entity_id: z6.string().uuid(),
  snapshot: z6.any(),
  // Keep broad - consuming code will validate by entity type
  author_id: z6.string().uuid().nullable(),
  created_at: z6.coerce.date()
}).strict();

// src/schemas/billing.ts
import { z as z7 } from "zod";
var BillingSchema = z7.object({
  profile_id: z7.string().uuid(),
  plan: z7.enum([PLAN.free, PLAN.pro]).default(PLAN.free),
  current_period_end: z7.coerce.date().nullable(),
  status: z7.enum([BILLING_STATUS.active, BILLING_STATUS.past_due, BILLING_STATUS.canceled, BILLING_STATUS.incomplete]).optional(),
  updated_at: z7.coerce.date()
}).strict();
